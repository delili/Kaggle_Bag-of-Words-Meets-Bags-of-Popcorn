...
if __name__ == '__main__':
    ...
    result = forest.predict_proba(testDataVecs)[:,1]
    ...
    result = model.predict_proba(testDataVecs)[:,1]
    output = pd.DataFrame( data={"id":test["id"], "sentiment":result} )
    
    
    output = pd.DataFrame( data={"id":test["id"], "sentiment":result} )
    output.to_csv( "Word2Vec_AverageVectors_proba11.csv", index=False, quoting=3 )
    print "Wrote Word2Vec_AverageVectors.csv"
